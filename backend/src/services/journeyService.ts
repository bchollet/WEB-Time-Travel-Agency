...
import {createJourney, deleteJourney, findAllJourneys, findJourneyById} from "../dataServices/journeyDataService";

export async function getAllJourneys(): Promise<Journey[]> {
    return await findAllJourneys()
        .then((daos: JourneyDAO[]): Journey[] => {
            return daos.map((dao: JourneyDAO): Journey => toDTO(dao));
        });
}

export async function getJourneyById(id: number): Promise<Journey> {
    return await findJourneyById(id)
        .then((dao: JourneyDAO): Journey => {
            return toDTO(dao);
        });
}

export async function addJourney(journey: Journey) {
    return await createJourney(toDAO(journey))
        .then((dao: JourneyDAO) => {
...
function toDTO(dao: JourneyDAO): Journey {
    return {
        id: dao.id,
...
function toDAO(dto: Journey): JourneyDAO {
    return {
